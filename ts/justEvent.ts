import type { JustDuo, JustEmpty, JustMeta, JustUno } from '@just-func/types'
import { type EventEmitterLike, type EventTargetLike, isEventEmitterLike, isEventTargetLike } from './types.js'

/**
 * Event with Value and Meta
 */
export interface JustEventDuo<Type extends string, Value, Meta extends JustMeta> {
	/**
 ...
 */
export interface JustEventUno<Type extends string, Value> {
	/**
	 * The event type: `emitter.emit(event.type)`
	 */
	type: Type
 ...
 * Event with no value
 */
export interface JustEventEmpty<Type extends string> {
	/**
	 * The event type: `emitter.emit(event.type)`
	 */
	type: Type
 ...
 * Creates just an event that is typed and easy to use.
 */
export function justEvent(
	type: string,
	defaultListener: (value: void) => unknown
): JustEventEmpty<typeof type> & {
	/**
 ...
 * Creates just an event that is typed and easy to use.
 */
export function justEvent<Value>(
	type: string,
	defaultListener: (value: Value) => unknown
): JustEventUno<typeof type, Value> & {
	/**
	 * The default event listener
	 */
	defaultListener: (value: Value) => unknown
 ...
 * Creates just an event that is typed and easy to use.
 */
export function justEvent<Value, Meta extends JustMeta>(
	type: string,
	defaultListener: (value: Value, meta: Meta) => unknown
): JustEventDuo<typeof type, Value, Meta> & {
	/**
	 * The default event listener
	 */
	defaultListener: (value: Value, meta: Meta) => unknown
...
export function justEvent<Value, Meta extends JustMeta>(type: string, defaultListener?: (...args: any[]) => any): any {
	return Object.assign(
		function event(value: Value, meta: Meta): JustDuo<Value, Meta> {
			return [value, meta]
		},
		{
			type,
			defaultListener,
			listener(callback: (...args: any[]) => any) {
				return callback
			},
