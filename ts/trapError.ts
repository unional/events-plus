import type { EventEmitterLike, EventTargetLike } from './types.js'
...
function overrideListener(target: any, logger: ErrorLogger, methodName: string) {
	...
	target[methodName] = (eventName: string | symbol, listener: (...args: any[]) => void) => {
...
function overrideEmit(target: any, logger: ErrorLogger, methodName: string) {
	...
	target[methodName] = (eventName: string | symbol, ...args: any[]) => {
		try {
			fn.call(target, eventName, ...args)
		} catch (e) {
			logger.error(e)
		}
	}
